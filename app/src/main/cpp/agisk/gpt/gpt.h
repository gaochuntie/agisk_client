...
enum GPTValidity {
    gpt_valid, gpt_corrupt, gpt_invalid
};

// Which set of partition data to use
enum WhichToUse {
    use_gpt, use_mbr, use_bsd, use_new, use_abort
};
...
public:
    ...
    int OnePartToMBR(uint32_t gptPart,
    ...
    uint64_t GetTableSizeInSectors(void) {
        return (((numParts * GPT_SIZE) / blockSize) +
